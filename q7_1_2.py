...
valid_set = datasets.MNIST('./data', train=False, transform=transforms.Compose([
                           transforms.ToTensor(),
                           transforms.Normalize((0.1307,), (0.3081,))
                       ]))
...
optimizer = optim.SGD(net.parameters(), lr=4e-3, momentum=0.9)
step_scheduler = lr_scheduler.MultiStepLR(optimizer, [60], gamma=0.1)
...
torch.save(best_model, 'best_model_lenet.pth')
